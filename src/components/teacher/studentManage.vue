...
export default {
  ...
  methods: {
    getStudentInfo() {
      //分页查询所有试卷信息
      this.$axios(`http://101.132.187.253:9000/api/students/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    checkGrade(studentId) { //修改学生信息
      ...
      this.$axios(`http://101.132.187.253:9000/api/student/${studentId}`).then(res => {
    ...
    deleteById(studentId) { //删除当前学生
      this.$confirm("确定删除当前学生吗？删除后无法恢复","Warning",{
        ...
        this.$axios({
          url: `http://101.132.187.253:9000/api/student/${studentId}`,
          method: 'delete',
        }).then(res => {
          this.getStudentInfo()
        })
    ...
    submit() { //提交更改
      ...
      this.$axios({
        url: 'http://101.132.187.253:9000/api/student',
        method: 'put',
        data: {
          ...this.form
        }
