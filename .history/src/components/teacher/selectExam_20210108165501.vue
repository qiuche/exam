...
export default {
  ...
  methods: {
    edit(examCode) { //编辑试卷
      ...
      this.$axios(`http://101.132.187.253:9000/api/exam/${examCode}`).then(res => { //根据试卷id请求后台
    ...
    submit() { //提交修改后的试卷信息
      ...
      this.$axios({
        url: 'http://101.132.187.253:9000/api/exam',
        method: 'put',
        data: {
          ...this.form
        }
    ...
    deleteRecord(examCode) {
      this.$confirm("确定删除该记录吗,该操作不可逆！！！","删除提示",{
        ...
        this.$axios({
          url: `http://101.132.187.253:9000/api/exam/${examCode}`,
          method: 'delete',
        }).then(res => {
          this.getExamInfo()
        })
    ...
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`http://101.132.187.253:9000/api/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
