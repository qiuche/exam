  ...
  methods: {
    //获取当前系统时间
     checkDate(){
      ...
      var startDate = +new Date(Date.parse(this.examData.examDate));//考试开始转换，转换
      // var startDate = +new Date('2021-1-8');//考试开始转换，转换
      var date = new Date(); 
      var seperator1 = "-";
      var year = date.getFullYear();//当前年
      var month = date.getMonth() + 1;//当前月
      var day = date.getDate();//当前日
      var currentdate = year + seperator1 + month + seperator1 + day;//变量currentdate 当前日期的字符串，下面直接转换类型比较
      var nowDate = +new Date(currentdate);//当前系统日期
      //判断
      var time = nowDate - startDate;
      if(time == 0){
          this.flag =  true;
      }else if(time < 0){
        alert('考试时间未到无法参加考试');
