     ...
     <el-dialog
        ...
        <span>请输入验证码：</span>
          <el-form class="login-form" status-icon :rules="loginRules" ref="loginForm" :model="loginForm" label-width="0">
            ...
            <el-form-item>
              <el-button type="primary" size="small" @click.native.prevent="handleLogin" class="login-submit">确定</el-button>
            </el-form-item>
          </el-form>
    </el-dialog>
...
import SIdentify from '@/components/student/identify.vue'
export default {
  store,
  data() {
    // 验证码自定义验证规则
  ...
  handleLogin() {
    this.$axios(`/api/exam/${examCode}`).loginForm.validate(valid => {
    if (valid) {
      alert('正确');
    }
    })
  ...
  // 生成四位随机验证码
    ...
    banchange(){
      ...
      this.$axios(`/api/exam/${examCode}`).then(res => {
        let count = 0;
        var divobj = document.getElementById('answer');
        history.pushState(null, null, document.URL);
        //禁用后退
        divobj.addEventListener('popstate', function () {
          history.pushState(null, null, document.URL);
        });
        //检测鼠标
        divobj.addEventListener('mouseleave',() => {
          count++;
          alert('考试过程中请勿切换页面！您已切出页面'+count+'次，超过三次将自动提交试卷');
          if(count == 3){
            alert('您已切出考试页面三次，将自动提交试卷');
      ...
      topic3Answer.forEach((element,index) => {
        ...
        if(right == this.topic[3][index].answer) { // 当前选项与正确答案对比
            ...
            //提交成绩信息
            this.$axios({
              url: 'http://101.132.187.253:9000/api/score',
              method: 'post',
              data: {
